        ...
        if($this->year && $this->month && $this->grade) {
            ...
            foreach($this->students as $student) {
                foreach(range(1, Carbon::create($this->year, $this->month)->daysInMonth) as $day) {
                    ...
                    $this->attendance[$student->id][$day] = Attendanc::where('student_id', $student->id)->whereDate('date', $date)->value('status') ?? 'present';
                }
            }
        }
    }
